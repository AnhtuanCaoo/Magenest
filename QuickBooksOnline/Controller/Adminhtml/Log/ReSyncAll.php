...
class ReSyncAll extends AbstractLog
{
    ...
    public function execute()
    {
        ...
        if ($connect && $connect == 1) {
            try {
                $count = 0;
                if ($this->config->isEnableByType('customer')) $count += $this->reSyncCustomer();
                if ($this->config->isEnableByType('item')) $count += $this->reSyncItem();
                if ($this->config->isEnableByType('order')) {
                    $count += $this->reSyncOrder();
                    $count += $this->reSyncOrderVoid();
                }
                if ($this->config->isEnableByType('invoice')) $count += $this->reSyncInvoice();
                if ($this->config->isEnableByType('creditmemo')) $count += $this->reSyncCreditmemo();
